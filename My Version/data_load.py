...
def _get_mfcc_and_spec(wav, preemphasis_coeff, n_fft, win_length, hop_length):
    ...
    # D = librosa.cqt(y=y_preem)
    # mag, phase = librosa.core.magphase(D)
    mag = np.abs(D)        
    #print(np.shape(mag))
    

    # Get mel-spectrogram
    mel_basis = librosa.filters.mel(hp.default.sr, hp.default.n_fft, hop_length)  # (n_mels, 1+n_fft//2) 
    #mel_basis, length = librosa.filters.constant_q(hp.default.sr)    
